...
**CLI:**
```
python -m synthetic.generate --archetypes --out ../data/loans            # the 15 goldens (fixed seed 42)
...
## 4. The 15 golden archetypes (FR-DAT-2) — committed at `data/loans/<name>.json`, seed 42
...
| 13 | `tx-a6-approve` | TX primary cash-out; LTV 78.0; lender fees 1.8%; no prior a6; notice on file; clean credit/DTI | base rules pass; STX-001..006 pass; STX-007 artifact set (TX-A6-DOCSET PTF) / Approve/Eligible / **approve_with_conditions** — golden asserts the TX document conditions + `not_applicable` records in non-TX overlay rules |
| 14 | `tx-a6-seasoning-decline` | TX primary cash-out; LTV 79.5 (passes base cash-out max 80 AND STX-001); prior a6 closed 320 days ago; notice on file; otherwise clean | STX-005 fail (320 < 365 days) / **decline** w/ RC-STATE-TX-50A6-SEASONING — demonstrates an overlay-only ineligibility on a fully base-eligible loan (T-SOV-3 most-restrictive-wins) |
| 15 | `ny-highcost-decline` | NY purchase; APR = Treasury + 8.5 (spread 8.500 > 8.000) | SNY-001 fail / **decline** w/ RC-STATE-HIGHCOST; SNY-003 not applicable (purchase) — golden asserts citation renders in packet |
...
- **Archetype mix:** 35% clean-approve variants, 15% conditional, 10% borderline-DTI, 10% self-employed/variable-income, 10% red-flag (deposit/occupancy), 12% decline family (credit/DTI/LTV/limit), 8% state-overlay family (TX a6 pass/fail, NY/MA/GA high-cost, CP-state NBS, FL insurance).
- **State distribution:** property states assigned from a fixed weighted list covering all overlay states (TX, NY, MA, GA, FL, CO, CA) plus no-overlay controls (OH, PA, MI); every overlay rule fires at least 5 times across the corpus and every overlay state also produces passing loans (the monitoring extract gets both outcomes per state).
